    ...
    def train(self, callbacks: DefaultDict[str, List[Callable]], **kwargs):
        ...
        if args.eval:
            test_stats, coco_evaluator = evaluate(
                model, criterion, postprocess, data_loader_val, base_ds, device, args, class_names=['TA','LV'])
            if args.output_dir:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            with torch.inference_mode():
                test_stats, coco_evaluator = evaluate(
                    model, criterion, postprocess, data_loader_val, base_ds, device, args=args, class_names=['TA','LV']
                )
            if not args.segmentation_head:
            ...
            if args.use_ema:
                ema_test_stats, _ = evaluate(
                    self.ema_m.module, criterion, postprocess, data_loader_val, base_ds, device, args=args, class_names=['TA','LV']
                )
        ...
        if args.run_test:
            ...
            test_stats, _ = evaluate(
                model, criterion, postprocess, data_loader_test, base_ds_test, device, args=args, class_names=['TA','LV']
            )
